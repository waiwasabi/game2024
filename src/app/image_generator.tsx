...
const Image_Generator = (props: PropsDefinition) => {
  ...
  return (
    ...
    // </div>
    <div className="p-4 bg-gray-900 rounded-lg max-w-2xl mx-auto mt-5">
  <form method="post" onSubmit={handleSubmit} className="space-y-4 flex items-end">
    <label className="block text-gray-300 font-medium flex-1">
      Prompt:
      <textarea
        name="prompt"
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        required
        className="w-11/12 p-3 mt-1 bg-gray-800 text-gray-200 rounded-lg resize-y min-h-[3rem] focus:outline-none focus:ring-2 focus:ring-blue-500"
        rows={1}
      ></textarea>
    </label>
    <button
